...
2. `EventAccumulator<E>` partitions internally by tenancyId. Events with different tenancyIds are never batched together. `drainIfReady(now)` changes return type from `List<LevelEvent<E>>` to `List<List<LevelEvent<E>>>` — returning all ready tenant partitions' batches at once, each list tenant-homogeneous. This aligns `EventAccumulator`'s drain pattern with `KeyedAccumulator.drain(long now)`, which already returns `List<List<LevelEvent<E>>>` (one list per ready group). `collect(LevelEvent)` is unchanged.
3. `KeyedAccumulator<K,E>` uses composite key `(tenancyId, K)` internally. Domain key extraction remains `Function<IN,K>` — tenancyId partitioning is transparent. The existing `drain(long now)` return type (`List<List<LevelEvent<E>>>`) is already correct.
4. `SummarisationRunner.tick(long now)` iterates the `List<List<LevelEvent<E>>>` from `drainIfReady(now)`, processing each tenant-homogeneous batch independently. This mirrors how `KeyedSummarisationRunner.tick()` already processes groups from `KeyedAccumulator.drain()`. Each output `LevelEvent` carries the batch's tenancyId.
...
- **Transition precedence:** When multiple transitions from the same state match (e.g., both `NORMAL→CONGESTION` and `NORMAL→EMERGENCY` fire), **first-match wins** — the first matching transition in declaration order is applied. This is consistent with `ExpressionRulesGanglion.detect()`, which iterates rules in order and returns on first match. YAML authors control priority through ordering.
- **Per-tenant state:** `phase-detect` maintains internal state (current phase) keyed by `tenancyId`. Each tenant has its own independent phase — Tenant A can be in CONGESTION while Tenant B is in NORMAL. The tenancyId is extracted from `batch.get(0).tenancyId()` (batches are guaranteed tenant-homogeneous by the accumulator). Null tenancyId uses a sentinel key (single-tenant mode).
- **State persistence:** `phase-detect` state is in-memory only. On pipeline restart (redeployment, crash), all tenant phases reset to the initial state. This is a known limitation — RAS provides durable detection state via `SituationContext` + `SituationStore` with `storeVersion`-based optimistic locking. The summarisation pipeline produces event signals; RAS owns durable situation tracking. If durable phase state is needed, it should be a follow-on enhancement (file as a child issue).

**Contract for stateful summarisers (Tier 2):**
1. Batches passed to `Summariser.summarise()` are guaranteed tenant-homogeneous — all events in the batch have the same `tenancyId`.
2. Stateful summarisers (those that track state across batches) MUST key their internal state by `batch.get(0).tenancyId()`. A single summariser instance handles all tenants; mixing tenant state produces incorrect results.
3. Null `tenancyId` is valid (single-tenant / test mode) — use a sentinel key for the state map.
